...
std::shared_ptr<lynx::runtime::MTSRuntime> GetVMContent(
    const EncoderOptions& encoder_options, runtime::ContextType ctx_type) {
  std::shared_ptr<lynx::runtime::MTSRuntime> vm_context(
      lynx::runtime::MTSRuntime::CreateContext(ctx_type));
  ...
  if (vm_context && vm_context->IsVMContext()) {
    ...
    // only opt lepus bytecode when sdk version is greater than or equal to 3.8
    // and opt bytecode is enabled
...
lynx::tasm::EncodeResult encode_ssr_inner(const uint8_t* ptr, size_t buf_len,
  ...
  std::shared_ptr<lynx::runtime::MTSRuntime> vm_context(
      lynx::runtime::MTSRuntime::CreateContext(
...
const char* lepusCheck(const char* sourceFile, char* targetSdkVersion,
  ...
  std::shared_ptr<lynx::runtime::MTSRuntime> context =
      lynx::runtime::MTSRuntime::CreateContext(
          lynx::runtime::ContextType::VMContextType);
  context->Initialize();
