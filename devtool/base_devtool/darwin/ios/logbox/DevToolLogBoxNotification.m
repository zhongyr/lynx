...
NSString *const DevToolLogBoxLevelError = @"error";
NSString *const DevToolLogBoxLevelWarn = @"warn";

@interface DevToolLogBoxNotificationManager ()

- (void)hideNotificationOfIndex:(NSInteger)index;

@end

#pragma mark - DevToolLogNotificationView

@interface DevToolLogNotificationView : UIView

@property(nonatomic, readwrite) NSInteger currentMsgCount;
@property(nonatomic, readwrite) NSInteger layoutIndex;

- (instancetype)initWithManager:(DevToolLogBoxNotificationManager *)manager
...
@implementation DevToolLogNotificationView {
  UILabel *_textLabel;
  UILabel *_countLabel;
...
#endif
  NSString *_level;
  __weak DevToolLogBoxNotificationManager *_manager;
}
...
- (instancetype)initWithManager:(DevToolLogBoxNotificationManager *)manager
  ...
  UIColor *levelColor;
  if ([_level isEqualToString:DevToolLogBoxLevelWarn]) {
    levelColor = warnColor;
  } else if ([_level isEqualToString:DevToolLogBoxLevelError]) {
...
#pragma mark - DevToolLogBoxNotificationManager
@implementation DevToolLogBoxNotificationManager {
  NSMutableDictionary<NSString *, DevToolLogNotificationView *> *_notificationViews;
  NSMutableDictionary<NSString *, NSString *> *_tapSelectors;
...
- (instancetype)initWithLogBoxManager:(DevToolLogBoxManager *)manager {
  ...
  if (self) {
    ...
    [_tapSelectors setObject:@"tapWarnNotification" forKey:DevToolLogBoxLevelWarn];
    [_tapSelectors setObject:@"tapErrorNotification" forKey:DevToolLogBoxLevelError];
  }
  return self;
}

- (void)showNotificationWithMsg:(NSString *)msg withLevel:(NSString *)level {
  if ([self getNotificationViewWithLevel:level] == nil) {
    DevToolLogNotificationView *view = [[DevToolLogNotificationView alloc] initWithManager:self
                                                                                 withLevel:level];
    SEL tapSel = NSSelectorFromString([_tapSelectors objectForKey:level]);
...
- (void)showMessage:(NSString *)msg withLevel:(NSString *)level {
  [[self getNotificationViewWithLevel:level] showMessage:msg];
}

- (void)showNotificationWithLevel:(NSString *)level {
  DevToolLogNotificationView *view = [self getNotificationViewWithLevel:level];
...
- (void)updateNotificationMsg:(NSString *)msg withLevel:(NSString *)level {
  if (msg != nil) {
...
- (void)updateNotificationMsgCount:(NSNumber *)count withLevel:(NSString *)level {
  if (count != nil) {
...
- (void)removeNotificationWithLevel:(NSString *)level {
  ...
  [_notificationViews removeObjectForKey:level];
}
...
- (DevToolLogNotificationView *)getNotificationViewWithLevel:(NSString *)level {
  @synchronized(_notificationViews) {
    return [_notificationViews objectForKey:level];
  }
}
